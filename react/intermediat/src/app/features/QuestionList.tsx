...
import IQuestion, { IAnswer } from "../domain/AssistantTypes";
...
const QuestionList = ({ isVisible = false }) => {
  ...
  return (
    <Paper elevation={0} sx={{ my: { md: 4, sm: 1 }, backgroundColor: 'white' }}>
      {items && items.map((quest, number, a) => (
        <FormControl fullWidth variant="standard" sx={{ m: { md: 2, sm: 1 }, mb: { md: 3, xs: 3 } }} className={'error_' + number}>
      ...
      <Grid container spacing={4}>
        ...
        <Grid md={4} sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
